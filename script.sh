...
pacstrap /mnt amd-ucode base tree kitty base-devel android-tools pulsemixer bluez bluez-utils ly efibootmgr chromium git grub htop intel-ucode linux linux-firmware linux-headers mpv micro net-tools networkmanager ntfs-3g obs-studio openssh papirus-icon-theme pipewire pipewire-alsa pipewire-jack pipewire-media-session pipewire-pulse unzip ttc-iosevka hyprland hyprpaper &&
